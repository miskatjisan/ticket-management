@php
    $user = auth()->user();
@endphp
<div class="user-profile" id="profileCoverImage" style="background-image:url('{{ getImage(getFilePath('userProfile') . '/' . $member->cover_photo, null, 'cover-photo') }}')">
            ...
            <div class="col-12">
                <div class="user-profile__content">
                    <div class="user-profile__info">
                        ...
                        <ul class="list list--row flex-wrap user-profile__info-list justify-content-center">
                            <li>
                    ...
                    <div class="user-profile__img">
                        <img src="{{ getImage(getFilePath('userProfile') . '/' . $member->image, null, 'user') }}" alt="@lang('member')" class="user-profile__img-is" id="profilePicture" />
                        @if ($member->id == @$user->id)
                            <input type="file" name="image" id="profile-picture" accept=".png, .jpg, .jpeg" hidden>
                    ...
                    <div class="user-profile__links">
                        ...
                        @endphp
                        @if ($socialLink)
                            <ul class="list list--row flex-wrap social-list justify-content-center">
                                @if ($socialLink->facebook)
                                    <li>
                                        <a href="{{ $socialLink->facebook }}" class="t-link social-list__icon">
                                ...
                                @if ($socialLink->twitter)
                                    <li>
                                        <a href="{{ $socialLink->twitter }}" class="t-link social-list__icon">
                                ...
                                @if ($socialLink->linkedin)
                                    <li>
                                        <a href="{{ $socialLink->linkedin }}" class="t-link social-list__icon">
                                ...
                                @if ($socialLink->instagram)
                                    <li>
                                        <a href="{{ $socialLink->instagram }}" class="t-link social-list__icon">
                        ...
                        @endif
                        <ul class="list list--row flex-wrap user-profile__list justify-content-center">
                            <li>
                        ...
                        @if ($member->id != @$user->id)
                            <ul class="list list--row flex-wrap justify-content-center" style="--gap: 0.5rem">
                                <li>
                                    @php
                                        $follow = null;
                                        if (auth()->check()) {
                                    ...
                                    @endphp
                                    @if ($follow)
                                        <button class="user-profile__btn active unfollow" data-following_id="{{ $member->id }}" data-followers_route="{{ route('member.followings', $member->username) }}">@lang('Unfollow')</button>
                                    @else
                                        <button class="user-profile__btn follow" data-following_id="{{ $member->id }}" data-followers_route="{{ route('member.followings', $member->username) }}">@lang('Follow')</button>
                                    @endif
                                </li>
                            </ul>
                        @else
                            <input type="file" name="cover_photo" id="cover-image" accept=".png, .jpg, .jpeg" hidden>
        ...
        <div class="row">
            <div class="col-12">
                <div class="tab-menu__content">
                    <a href="{{ route('member.images', $member->username) }}" class="tab-menu__link {{ menuActive('member.images') }}"> @lang('IMAGES') </a>
                    <a href="{{ route('member.followers.followings', $member->username) }}" class="tab-menu__link {{ menuActive('member.followers.followings') }}"> @lang('FOLLWERS & FOLLWINGS') </a>
                    <a href="{{ route('member.collections', $member->username) }}" class="tab-menu__link {{ menuActive('member.collections') }}"> @lang('COLLECTION') </a>
                </div>
            </div>
        </div>
    </div>
