...
function fileUrl($file)
{
    $general = gs();
    if ($general->storage_type == 2) {
        return @$general->ftp->host_domain . '/files/' . $file;
    } else {
        $path      = getFilePath('stockFile');
        return $path . '/' . $file;
    }
}
...
function ftpFileUploader($file, $location, $size = null, $old = null, $thumb = null)
{
    $ftpFileManager = new FTPFileManager($file);
    $ftpFileManager->path = $location;
    $ftpFileManager->size = $size;
    $ftpFileManager->old = $old;
    $ftpFileManager->thumb = $thumb;
    $ftpFileManager->upload();
    return $ftpFileManager->filename;
}
