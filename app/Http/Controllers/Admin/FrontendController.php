        ...
        foreach ($request->except('_token', 'video') as $input_field => $val) {
            if ($input_field == 'has_image' && $imgJson) {
                foreach ($imgJson as $imgValKey => $imgJsonVal) {
                    $validation_rule['image_input.' . $imgValKey] = ['nullable', new FileTypeValidate(['jpg', 'jpeg', 'png'])];
                ...
                $validation_rule['image_input'] = ['nullable', new FileTypeValidate(['jpeg', 'jpg', 'png'])];
                continue;
            }
