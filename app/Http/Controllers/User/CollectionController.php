...
class CollectionController extends Controller
{
    ...
    public function imageData(Request $request)
    {
        ...
        $html = view($this->activeTemplate . 'partials.collection_content', compact('image'))->render();
    ...
    public function addImage(Request $request)
    {
        ...
        if ($request->collection) {
            ...
            foreach ($request->collection as $key => $collection) {
                $exist = CollectionImage::where('collection_id', $collection)->where('image_id', $request->image)->first();
